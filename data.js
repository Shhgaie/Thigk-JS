...
function listProducts() {
  if(localStorage.length > 0) {
    let room = localStorage.getItem("createRoom");
    console.log(room);
  }
    var demo = '';
...
function createProduct() {
    ...
    localStorage.setItem("createRoom", JSON.stringify({idRoom, name, price, firstDate, endDate, image}));
    console.log(listRoom);
  ...
  function deleteRoom(idDelete) {
    ...
    listRoom.forEach(function(item) {
      ...
      row.innerHTML = '<td>' + item.id + '</td><td>' + item.name + '</td><td>' + item.price + '</td><td>' + item.firstDate + '</td><td>' + item.endDate + '</td><td>' + '<a href="#" class="btn btn-primary" onclick="deleteRoom(' + item.id + ')">Xoá</a>' + '</td>'+ '<td>' + '<a href="#" class="btn btn-primary" onclick="edit()">Sửa</a>' + '</td>';
  ...
  function displayEditForm(id) {
    ...
    formHtml += '<input class="btn btn-primary" onclick="edit(' + id + ')" type="button" value="Lưu thay đổi">';
    formHtml += '</form> </div>';
  ...
  function edit(id) {
    ...
    let editStartDate = document.getElementById('editStartDate').value;
    let editEndDate = document.getElementById('editEndDate').value;
    let linkImage = document.getElementById('linkImage').value;
    let editList = listRoom.map((item) => {
      if(item.id == id) {
        item.name=editName;
        item.price=editPrice;
        item.firstDate=editStartDate;
